...
function addFeedback(productName, address, website) {
    client.query('INSERT INTO feedback (product_name, address, website) VALUES ($1, $2, $3)', 
        [productName, address, website], (err, res) => {
            if (err) {
                console.log(err.message);
            } else {
                console.log('Feedback data insert successful');
            }
        });
}
...
app.get('/feedbacks', async (req, res) => {
    try {
        const data = await client.query('SELECT * FROM feedback'); // Adjust table name as per your DB schema
        res.send(data.rows);
    } catch (error) {
...
app.post('/addfeedback', (req, res) => {
    const { productName, address, website } = req.body;
    addFeedback(productName, address, website);
    res.send('Feedback data inserted');
});
